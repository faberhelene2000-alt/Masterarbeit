...
west_final <- process_region_weighted(
  ...
  "Weststeiermark_Sorten_Skala_bereinigt.csv",
...
vulkan_final <- process_region_weighted(
  ...
  "Vulkanland_Sorten_Skala_bereinigt.csv",
...
model_nl <- lm(Detrended ~ frost_schaden + I(frost_schaden^2), data = final)
summary(model_nl)
...
# Vergleich Burning Cost 
burning_compare <- data.table(
  Variante = c("inkl_2017", "exkl_2017"),
  Praemie = c(
    payout_dt[, mean(payout_rate, na.rm = TRUE) * 100],
    payout_dt[year != 2017, mean(payout_rate, na.rm = TRUE) * 100]
  )
)

print(burning_compare)
...
), "Model_2_Aggregation_final.xlsx")
